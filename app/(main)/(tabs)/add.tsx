  ...
  const processImage = async (uri: string) => {
    try {
      ...
      if (data.metadata?.title || data.metadata?.author) {
        searchQuery = [data.metadata.title, data.metadata.author]
          .filter(Boolean)
          .join(' ');
    ...
    <View className="flex-1 bg-black">
      {isFocused ? (
        <>
          ...
          />
          <View style={StyleSheet.absoluteFillObject} className={`flex-col justify-between p-6 pb-28 pt-16 ${isProcessing ? 'bg-black/30' : ''}`}>
